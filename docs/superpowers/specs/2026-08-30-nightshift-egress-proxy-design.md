...
- **Substrate / Kubernetes compute (Plan 5):** default-deny egress
  ...
  actor and must fail.
- The internal run-record API is an always-allowed destination through the
  proxy when the proxy is the only egress; on Local the harness continues to
  call it directly.
...
```
harness (in actor)
   │  bearer: run JWT          base URLs from llm.Config point here
   ▼
proxy (ours)  /proxy/llm/{provider}/{path...}
   │  1 authenticate (run JWT + stored-hash check, as internal API)
   ...
   │  6 append run event (request/denied/error)
   ▼
provider (TLS, system roots)
```
...
type CredentialSource interface {
    // Credential returns the secret for a tenant's named connection, or the
    // platform default when name == "default" and the tenant has none.
...
- `llm.connection` — the vault connection supplying the key. `"default"`
  resolves to the tenant's BYO default if one exists, else the platform key.
...
`kind` starts with one value; connectors add `oauth` and friends.
...
- Tenant creation mints the KEK inside `CreateTenant`'s existing transaction.
  Pre-release, no deployed databases: dev databases are recreated, not
  backfilled.
...
1. **Authenticate** — bearer run-JWT verified with the same `token.Signer`
   plus the stored-hash check, identical rules to the internal API. Same
   binary today means no key-distribution problem; the standalone split later
   chooses between shared-key config and an introspection endpoint, decided at
   split time and noted here so it isn't rediscovered.
...
5. **Forward, streaming** — one `httputil.ReverseProxy` per provider,
   ...
   so there is no user-controlled destination to canonicalize and no
   redirect/DNS-rebinding surface in v1. That surface appears with connectors,
   and the connector-catalog spec owns it.
...
- `llm.Config` base URLs point at the proxy prefix; the ported provider code
  is untouched. `harness.Input.APIKey` is **removed**; the harness sets the
  literal placeholder `"proxied"` in `CallOptions` (the SDKs require a
  value), and the proxy strips it.
...
run token → 401 (clearing a run's token hash revokes proxy access at the next
auth check). Upstream provider errors and timeouts pass through untouched —
the harness's existing `llm_error` path and run finalization own them, so the
proxy adds no new harness error handling. The permit cache is per-run, bounded
by token expiry, evicted on finalize; no cross-run sharing.

## Observability

The run-event stream is the audit surface: `proxy.request`, `proxy.denied`,
`proxy.error` with provider/status/duration only. Redacted `slog` throughout
the proxy and vault. `connection.last_used_at` tells a tenant a key is in use.
...
destination canonicalization and redirect/DNS-rebinding defenses (no
user-controlled destinations exist in v1 — documented above); OAuth
